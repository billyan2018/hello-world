...
unshiftedSpecialKeys = '{,./;\'[]\\`-='
shiftedSpecialKeys = {["`"] = "~", ["1"] = "!", ["2"] = "@", ["3"] = "#", ["4"] = "$", ["5"] = "%", ["6"] = "^", ["7"] = "&", ["8"] = "*", ["9"] = "(", ["0"] = ")", ["-"] = "_", ["="] = "+", ["["] = "{", ["]"] = "}", [";"] = ":", ["'"] = '"', [","] = "<", ["."] = ">", ["/"] = "?"}
...
function console.draw()
	-- Only draw the console if enabled (disabled to make pure console app)
...
-- removed textinput since love.textinput not working (bug)
...
function console.execute(command)
	...
	local chunk, error = load("return " .. command)
		if string.match(command, "love.graphics.-%)") then
		for capture in string.gmatch(command, "love.graphics.-%)") do
...
function console.keypressed(key, scancode, isrepeat)
	-- Ignore if the console isn't enabled (removed for this console)
	...
	elseif #key == 1 then
		if not shift then
			if (string.byte(key) >= 48 and string.byte(key) < 58)  -- digits
				or (string.byte(key) >= 97 and string.byte(key) < 123)  -- lowercase letters
				or unshiftedSpecialKeys:find(key, --[[from index]] 1, --[[literal character]] true)
			then
				command:insert(key)
			end
		else
			if string.byte(key) >= 97 and string.byte(key) < 123 then  -- uppercase letters
				local newChar = string.byte(key) - 32
				command:insert(string.char(newChar))
			else
				for index, value in pairs(shiftedSpecialKeys) do
					if key == index then
